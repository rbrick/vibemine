...
public final class ApiReferenceTool implements Tool<ApiReferenceTool.ApiReferenceInput, ApiReferenceTool.ApiReferenceOutput> {
    ...
    private static Map<String, Class<?>> buildTypes() {
        ...
        register(types, VibeParticles.class, "particles", "particle");
    ...
    private static String notes() {
        return "Ask for a specific type, e.g. {\"type\":\"VibePlayer\"}, {\"type\":\"CommandExecutionContext\"}, {\"type\":\"database\"}, {\"type\":\"table\"}, {\"type\":\"particles\"}, or {\"type\":\"minimessage\"}. Global bindings include server, inventories, particles, permissions, scheduler, database, and minimessage.";
    }
    ...
    private static String notesFor(Class<?> type) {
        ...
        if (type == VibePlayer.class) return "VibePlayer extends VibeSender. Particles: spawnParticle(name, count, offsets..., extra), spawnParticleAt(location, name,...), and spawnDustParticle(location, count, r,g,b,size).";
        if (type == VibeWorld.class) return "World utilities include get/set block helpers, spawnEntity, lightning, playSound, spawnParticle(name, location, count, offsets..., extra), spawnParticleAt, and spawnDustParticle.";
        ...
        if (type == VibeParticles.class) return "Global particle helper. Use particles.names(), particles.exists(name), and particles.requiresData(name). Spawn with world.spawnParticle/player.spawnParticle. Particles that require data are blocked except dust via spawnDustParticle.";
